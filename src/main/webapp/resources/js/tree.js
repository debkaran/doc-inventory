...
function TreeView(root, container, fun1, options){
	var self = this;
	...
	*/
	...
	function renderNode(node){
		...
		span_desc.addEventListener("click", function(e){
			...
			if(typeof fun1 !== "undefined"  && fun1 != null){
				fun1();
			}
		});
		...
		if(node.isLeaf() && !TreeUtil.getProperty(node.getOptions(), "forceParent", false)){
			...
			if(icon != ""){
				...
				ret += '<span class="tj_icon">' + TreeConfig.parent_close_icon + '</span>';
			}
...
const TreeUtil = {
	...
	default_parent_close_icon: "<span>&#128448;</span>",
	...
	getFullPath: function(node){
		...
		TreeUtil.getSelectedNodesForNode(node).forEach(function(_node){
			lastSelectedNode = _node;
		});
		ret.push(lastSelectedNode);
		while(typeof lastSelectedNode.parent !== "undefined"){
			ret.push(lastSelectedNode.parent);
...
var TreeConfig = {
	...
	parent_close_icon: TreeUtil.default_parent_close_icon,
